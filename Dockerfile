...
RUN apt-get update \
    && set -x \
    && apt-get install -qq --no-install-recommends -y apt-utils build-essential ca-certificates cmake cuda-core-9-0 git cuda-cudart-dev-9-0 libhwloc-dev libmicrohttpd-dev libssl-dev \
